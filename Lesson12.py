...
def drawScrew(sPos = vector(0, 0, 0), scale = 1, axis = 'z'):
...
# TODO: Fully parameterise meterType1, meterType2, meterType3, meterType4.
...
class meterType1:
    def __init__(self, mt1Pos = vector(0, 0, 0), mt1Color = color.red, mt1ScaleMin = 0, mt1ScaleMax = 5, mt1Label = "", mt1Units = ""):
        ...
        # Corner screws.
        drawScrew(vector(-1.045, 0.67, 0.02) + self.mt1Pos)  # Top Left corner.
        drawScrew(vector(1.045, 0.67, 0.02) + self.mt1Pos)   # Top Right corner.
        drawScrew(vector(-1.045, -0.67, 0.02) + self.mt1Pos) # Bottom Left corner.
        drawScrew(vector(1.045, -0.67, 0.02) + self.mt1Pos)  # Bottom Right corner.
        # Lets put a mostly transparent glass cover over the virtual meter.
        box(color = color.white, opacity = 0.25, size = vector(2.25, 1.5, 0.35), pos = vector(0, 0, 0.20) + self.mt1Pos)
...
class meterType2:
    def __init__(self, mt2Pos = vector(0, 0, 0), mt2Color = color.blue, mt2ScaleMin = 0, mt2ScaleMax = 100, mt2Label = "", mt2Units = ""):
        ...
        # Center screw.
        drawScrew(vector(0, 0, 0.02) + self.mt2Pos)
        # Lets put a mostly transparent glass cover over the virtual meter.
...
class meterType3:
    def __init__(self, mt3Pos = vector(0, 0, 0), mt3Color = color.red, mt3ScaleMin = 0.0, mt3ScaleMax = 100.0, mt3Label = "", mt3Units = ""):
        ...
        # Corner screws.
        drawScrew(vector(-0.3, 0.8, 0.02) + self.mt3Pos)  # Top Left corner.
        drawScrew(vector(0.3, 0.8, 0.02) + self.mt3Pos)   # Top Right corner.
        drawScrew(vector(-0.3, -0.8, 0.02) + self.mt3Pos) # Bottom Left corner.
        drawScrew(vector(0.3, -0.8, 0.02) + self.mt3Pos)  # Bottom Right corner.
        # Lets put a mostly transparent glass cover over the virtual meter.
...
# A simple LED - fully parameterised.
class simpleLED():
...
class rgbLEDBank():
    def __init__(self, rgbLEDBankPos = vector(0, 0, 0), rgbInARow = True, bgThreshold = 5, rgThreshold = 30, hysteresis = 0.5):
        ...
        if rgbInARow:  # Draw the LEDs horizontally.
            self.blueLED  = simpleLED(vector(-0.25, 0, 0) + rgbLEDBankPos)
            self.greenLED = simpleLED(vector(0, 0, 0) + rgbLEDBankPos)
            self.redLED   = simpleLED(vector(0.25, 0, 0) + rgbLEDBankPos)            
        else:               # Draw the LEDs vertically.
            self.blueLED  = simpleLED(vector(0, -0.25, 0) + rgbLEDBankPos)
            self.greenLED = simpleLED(vector(0, 0, 0) + rgbLEDBankPos)
            self.redLED   = simpleLED(vector(0, 0.25, 0) + rgbLEDBankPos)
    def update(self, sensorValue = "NAN"):
        # If we have valid data.
        if sensorValue != "NAN":
            if sensorValue < self.bgThreshold: # Under the blue-green threshold.
                self.blueLED.update(color.blue)
                self.greenLED.update()
                self.redLED.update()
            if self.bgThreshold <= sensorValue <= self.rgThreshold: # Within the thresholds for green.
                self.blueLED.update()
                self.greenLED.update(color.green)
                self.redLED.update()
            if sensorValue > self.rgThreshold: # Over the green-red threshold.
                self.blueLED.update()
                self.greenLED.update()
...
# A 3 discrete RGB color LED. TODO: Full parameterisation.
class rgbColorLED():
    def __init__(self, rgbColorLEDPos = vector(0, 0, 0), bgThreshold = 1.5, rgThreshold = 3.5, hysteresis = 0.5, offColor = color.gray(0.5)):
        ...
        # The LED off color that is used later.
        self.offColor = offColor
        ledDome = sphere(color = color.white, opacity = 0.95, radius = 0.1, shininess = 0.25, pos = vector(0, 0, 0.20) + rgbColorLEDPos)
        ledBulb = simple_sphere(color = color.white, opacity = 1 , radius = 0.08, shininess = 0.75, pos = vector(0, 0, 0.20) + rgbColorLEDPos)
        ledBody = cylinder(color = color.white, opacity = 0.95, radius = 0.1, shininess = 0.25, pos = vector(0, 0, 0.05) + rgbColorLEDPos, axis = vector(0, 0, 0.15))
        # The LED base is initially set to a very light gray so that it is always a bit darker than the rest of the LED.
        ledBase = cylinder(color = color.gray(0.90), opacity = 0.95, radius = 0.125, shininess = 0.25, pos = vector(0, 0, 0) + rgbColorLEDPos, axis = vector(0, 0, 0.05))
        cylinder(color = color.white, opacity = 1, radius = 0.01, pos = vector(-0.06, 0, 0.025) + rgbColorLEDPos, axis = vector(0, 0, -0.25))
        cylinder(color = color.white, opacity = 1, radius = 0.01, pos = vector(-0.02, 0, 0.025) + rgbColorLEDPos, axis = vector(0, 0, -0.30))
        cylinder(color = color.white, opacity = 1, radius = 0.01, pos = vector(0.02, 0, 0.025) + rgbColorLEDPos, axis = vector(0, 0, -0.35))
        cylinder(color = color.white, opacity = 1, radius = 0.01, pos = vector(0.06, 0, 0.025) + rgbColorLEDPos, axis = vector(0, 0, -0.25))
        self.rgbColorLED = compound([ledDome, ledBulb,ledBody, ledBase], color = self.offColor) # We want to change the color of these parts simultaneously.
    def update(self, sensorValue = "NAN"):
        # If we have valid data.
        if sensorValue != "NAN":
            self.rgbColorLED.emissive = True
            if sensorValue < self.bgThreshold: # Under the blue-green threshold.
                self.rgbColorLED.color = color.blue
            if self.bgThreshold <= sensorValue <= self.rgThreshold: # Within the thresholds for green.
                self.rgbColorLED.color = color.green
            if sensorValue > self.rgThreshold: # Over the green-red threshold.
                self.rgbColorLED.color = color.red
        else:
            self.rgbColorLED.emissive = False
            self.rgbColorLED.color = self.offColor

# A full color range RGB tricolor LED. TODO: Full parameterisation.
class rgbTriColorLED():
    def __init__(self, rgbTriColorLEDPos = vector(0, 0, 0), offColorR = 127, offColorG = 127, offColorB = 127):
        # The LED off colors that are used later.
        ...
        ledDome = sphere(color = color.white, opacity = 0.95, radius = 0.1, shininess = 0.25, pos = vector(0, 0, 0.20) + rgbTriColorLEDPos)
        ledBulb = simple_sphere(color = color.white, opacity = 1 , radius = 0.08, shininess = 0.75 , pos = vector(0, 0, 0.20) + rgbTriColorLEDPos)
        ledBody = cylinder(color = color.white, opacity = 0.95, radius = 0.1, shininess = 0.25, pos = vector(0, 0, 0.05) + rgbTriColorLEDPos, axis = vector(0, 0, 0.15))
        # The LED base is initially set to a very light gray so that it is always a bit darker than the rest of the LED.
        ledBase = cylinder(color = color.gray(0.90), opacity = 0.95, radius = 0.125, shininess = 0.25, pos = vector(0, 0, 0) + rgbTriColorLEDPos, axis = vector(0, 0, 0.05))
        cylinder(color = color.white, opacity = 1, pos = vector(-0.06, 0, 0.025) + rgbTriColorLEDPos, axis = vector(0, 0, -0.25), radius = 0.01)
        cylinder(color = color.white, opacity = 1, pos = vector(-0.02, 0, 0.025) + rgbTriColorLEDPos, axis = vector(0, 0, -0.30), radius = 0.01)
        cylinder(color = color.white, opacity = 1, pos = vector(0.02, 0, 0.025) + rgbTriColorLEDPos, axis = vector(0, 0, -0.35), radius = 0.01)
        cylinder(color = color.white, opacity = 1, pos = vector(0.06, 0, 0.025) + rgbTriColorLEDPos, axis = vector(0, 0, -0.25), radius = 0.01)
        self.rgbTriColorLED = compound([ledDome, ledBody, ledBase], color = self.offColor) # We want to change the color of these parts simultaneously.
    def update(self, rgbTriColorLEDR = "off", rgbTriColorLEDG = "off", rgbTriColorLEDB = "off"):
        rgbColorLEDEmissive = False # This will be changed later if any of the colours are on.
        if rgbTriColorLEDR == "off":
            self.colorR = self.offColorR
        else:
            rgbColorLEDEmissive = True
            self.colorR = rgbTriColorLEDR / 255
        if rgbTriColorLEDG == "off":
            self.colorG = self.offColorG
        else:
            rgbColorLEDEmissive = True
            self.colorG = rgbTriColorLEDG / 255
        if rgbTriColorLEDB == "off":
            self.colorB = self.offColorB
        else:
            rgbColorLEDEmissive = True
            self.colorB = rgbTriColorLEDB / 255
        self.rgbColorLED.emissive = rgbColorLEDEmissive
        self.rgbTriColorLED.color = vector(self.colorR, self.colorG, self.colorB)
...
drawScrew(vector(-3.4, 1.9, -0.18))
drawScrew(vector(3.4, 1.9, -0.18))
drawScrew(vector(-3.4, -2.4, -0.18))
drawScrew(vector(3.4, -2.4, -0.18))
